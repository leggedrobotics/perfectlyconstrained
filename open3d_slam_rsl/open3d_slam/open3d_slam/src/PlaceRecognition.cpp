...
Constraints PlaceRecognition::buildLoopClosureConstraints(const Transform& mapToRangeSensor, const SubmapCollection& submapCollection,
                                                          const AdjacencyMatrix& adjMatrix, size_t lastFinishedSubmapIdx,
  ...
  const std::vector<size_t> closeSubmapsIdxs =
      std::move(getLoopClosureCandidatesIdxs(mapToRangeSensor, submapCollection, adjMatrix, lastFinishedSubmapIdx, activeSubmapIdx));
  std::cout << "considering submap " << lastFinishedSubmapIdx << " for loop closure, num candidate submaps: " << closeSubmapsIdxs.size()
            << std::endl;
  using namespace open3d::pipelines::registration;
  ...
  for (int i = 0; i < closeSubmapsIdxs.size(); ++i) {
    ...
    RegistrationResult ransacResult;
    {
      ...
      ransacResult = RegistrationRANSACBasedOnFeatureMatching(
          sourceSparse, targetSparse, sourceFeature, targetFeature, true, cfg.ransacMaxCorrespondenceDistance_,
          TransformationEstimationPointToPoint(false), cfg.ransacModelSize_, {distanceChecker, edgeLengthChecker},
    ...
    if (ransacResult.correspondence_set_.size() < cfg.ransacMinCorrespondenceSetSize_) {
      std::cout << "REJECTED loop closure, " << ransacResult.correspondence_set_.size() << " correspondences. " << matchingSubmapsString
                << "\n";
      continue;
    }
    ...
    const double mapVoxelSize = getMapVoxelSize(params_.mapBuilder_, magic::voxelSizeCorrespondenceSearchIfMapVoxelSizeIsZero);
    ...
    computeIndicesOfOverlappingPoints(source, target, Transform(ransacResult.transformation_), voxelSizeForOverlap, minNumPointsPerVoxel,
    ...
    c.informationMatrix_ = open3d::pipelines::registration::GetInformationMatrixFromPointClouds(
        sourceOverlap, targetOverlap, cfg.maxIcpCorrespondenceDistance_, icpResult.transformation_);
    ...
    if (params_.placeRecognition_.isDumpPlaceRecognitionAlignmentsToFile_) {
      std::string lcName =
          std::to_string(recognitionCounter_) + "_" + std::to_string(sourceSubmap.getId()) + "_" + std::to_string(targetSubmap.getId());
...
bool PlaceRecognition::isRegistrationConsistent(const Eigen::Matrix4d& mat) const {
  ...
  if (std::fabs(roll) > p.maxDriftRoll_) {
    result = false;
    std::cout << "  PlaceRecognition::isRegistrationConsistent The roll drift is: " << roll * kRadToDeg << " [deg] which is > than "
              << p.maxDriftRoll_ * kRadToDeg << "\n";
  }
  if (std::fabs(pitch) > p.maxDriftPitch_) {
    result = false;
    std::cout << "  PlaceRecognition::isRegistrationConsistent The pitch drift is: " << pitch * kRadToDeg << " [deg] which is > than "
              << p.maxDriftPitch_ * kRadToDeg << "\n";
  }
  if (std::fabs(yaw) > p.maxDriftYaw_) {
    result = false;
    std::cout << "  PlaceRecognition::isRegistrationConsistent The yaw drift is: " << yaw * kRadToDeg << " [deg] which is > than "
              << p.maxDriftYaw_ * kRadToDeg << "\n";
  }
  if (std::fabs(T.translation().x()) > p.maxDriftX_) {
    result = false;
    std::cout << "  PlaceRecognition::isRegistrationConsistent The x drift is: " << T.translation().x() << " [m] which is > than "
              << p.maxDriftX_ << "\n";
  }
  if (std::fabs(T.translation().y()) > p.maxDriftY_) {
    result = false;
    std::cout << "  PlaceRecognition::isRegistrationConsistent The y drift is: " << T.translation().y() << " [m] which is > than "
              << p.maxDriftY_ << "\n";
  }
  if (std::fabs(T.translation().z()) > p.maxDriftZ_) {
    result = false;
    std::cout << "  PlaceRecognition::isRegistrationConsistent The z drift is: " << T.translation().z() << " [m] which is > than "
              << p.maxDriftZ_ << "\n";
  }
...
std::vector<size_t> PlaceRecognition::getLoopClosureCandidatesIdxs(const Transform& mapToRangeSensor,
  ...
  for (size_t i = 0; i < nSubmaps; ++i) {
    ...
    const bool isAdjacent = std::abs<int>(i - lastFinishedSubmapIdx) == 1 || adjMatrix.isAdjacent(i, lastFinishedSubmapIdx);
    if (isAdjacent) {
    ...
    if (loopClosingDistance < params_.placeRecognition_.minSubmapsBetweenLoopClosures_) {
      std::cout << "Skipping the loop closure of " << matchingSubmapsString << " since there are fewer than "
                << params_.placeRecognition_.minSubmapsBetweenLoopClosures_ << " submaps inbetween \n";
      continue;
    }
